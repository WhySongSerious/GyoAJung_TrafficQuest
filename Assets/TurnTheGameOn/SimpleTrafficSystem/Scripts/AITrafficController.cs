namespace TurnTheGameOn.SimpleTrafficSystem
{
    ...
    public class AITrafficController : MonoBehaviour
    {
        ...
        private QueryParameters queryParameters = QueryParameters.Default;
        ...
        private void FixedUpdate()
        {
            if (isInitialized)
            {
                ...
                for (int i = 0; i < carCount; i++) // operate on results
                {
                    ...
                    frontBoxcastCommands[i] = new BoxcastCommand(frontSensorTransformPositionNL[i], frontSensorSizeNL[i], frontRotationList[i], frontDirectionList[i], queryParameters, frontSensorLengthNL[i]);
                    ...
                    if (useLaneChanging)
                    {
                        if (speedNL[i] > minSpeedToChangeLanes)
                        {
                            if ((forceChangeLanesNL[i] == true || frontHitNL[i] == true) && canChangeLanesNL[i] && isChangingLanesNL[i] == false)
                            {
                                ...
                                leftBoxcastCommands[i] = new BoxcastCommand(leftOriginList[i], sideSensorSizeNL[i], leftRotationList[i], leftDirectionList[i], queryParameters, sideSensorLengthNL[i]);
                                ...
                                rightBoxcastCommands[i] = new BoxcastCommand(rightOriginList[i], sideSensorSizeNL[i], rightRotationList[i], rightDirectionList[i], queryParameters, sideSensorLengthNL[i]);
                            }
                        }
                    }
                }
